...
(RunTime Weaving(AspectJ는 컴파일 시점), CGLib은 런타임 시점)<br>
...
[운영계 티니패스 이용내역조회 쿼리 실행시간 지연이슈 해결]

기존 1.0에는 로그인한 사용자가 소유한 모든 카드에 대한 이용내역조회만 있어서 PK인 카드번호를 WHERE로 가진 SELECT 쿼리문을 호출했었습니다. 현 프로젝트에서는 티니패스 카드 또한 이용내역조회가 가능해져 기존 테이블에 다른 티니패스 카드 테이블과 N:1으로 바라보는 GFT_CDNO(티니패스카드번호) FK 컬럼을 추가하고 해당 컬럼을 WHERE로 SELECT 하는 쿼리를 만들어 로직을 구현했었습니다.
단위테스트 및 통합테스트 당시 개발계 및 검증계는 운영계 전체 데이터의 10% 정도만 제공되기 때문에, 해당 쿼리에 대해 속도 지연 이슈가 없었지만 CBT에서 운영계로 테스트한 결과 10초 정도 지연이 발생했습니다.

이에 대해 원인을 분석한 결과, 위에 말한 것과 같이 데이터 개수에 따른 속도 지연이 아니었습니다.
개발 당시 PL/SQL로 작성한 쿼리에 대해 실행계획을 확인했을 때 개발계 및 검증계 DB에는 해당 컬럼에 인덱싱이 적용되있어서 INDEX SCAN을 탔는데
운영계 DB에는 별도 Indexing이 처리를 하지 않아 FULL TABLE SCAN을 타고 있었습니다.

따라서, 운영계 DB에 해당 컬럼에 대해 Indexing이 처리를 한 후 재테스트한 결과 어느정도 속도는 줄였지만, 여전히 사용자가 느끼기에는 느린 속도였습니다.

이를 해결하기 위해 Oracle에서 사용할 수 있는 Hint를 적용했습니다.
해당 테이블에는 PK 뿐 아니라 여러 컬럼 각각에도 Indexing이 되어있고, 다른 테이블들과 INNER JOIN을 하고 있어 인덱싱을 잘 못 탈 때가 있었습니다.
따라서, 해당 컬럼의 인덱스를 좀 더 정확히 타기 위해 / + INDEX(Table_name(alias) Index_name) / (주석 +인덱스 테이블별칭 인덱스 테이블이름) 을 SELECT 바로 다음에 기재해 Oracle Hint를 사용했고,
그 결과 0.1초 내외로 속도를 개선시켜 자칫하면 커질 수 있었던 문제를 해결할 수 있었습니다.

이를 통해 쿼리를 짜거나 DB 튜닝할 때 SQL 실행계획을 통해 실제 쿼리가 DB에 어떻게 동작되는지 점검하는 습관을 유지하는 것과 
적절한 상황일 때 Indexing과 hint를 사용하여 조회 속도를 개선시키는 것을 깊게 자리잡게 되었습니다.
...
### 사이드 프로젝트 검색 로그 저장 동시성 이슈(검색어, 검색횟수)  

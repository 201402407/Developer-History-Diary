...
기존 1.0에는 로그인한 사용자가 소유한 모든 카드에 대한 이용내역조회만 있어서 DB 테이블에 데이터를 저장할 때 PK인 카드번호를 WHERE로 가진 SELECT 쿼리문을 호출했었습니다. 현 프로젝트에서는 티니패스 카드 또한 이용내역조회가 가능해져 같은 테이블에 GFT_CDNO(티니패스카드번호) 컬럼을 추가하여 해당 컬럼을 WHERE로 SELECT 하는 쿼리를 만들어 로직을 구현했었습니다.
단위테스트 및 통합테스트 당시 개발계 및 검증계는 운영계 전체 데이터의 10% 정도만 제공되기 때문에, 해당 쿼리에 대해 속도 지연 이슈가 없었지만 CBT에서 운영계로 테스트한 결과 10초 이상의 지연이 발생했습니다.

이에 대해 원인을 분석한 결과, 위에 말한 것과 같이 데이터 개수에 따른 속도 지연이 아니었습니다.
개발 당시 PL/SQL로 작성한 쿼리에 대해 실행계획을 확인한 결과 개발계 및 검증계 DB에는 해당 컬럼에 인덱싱이 적용되있어서 INDEX SCAN을 탔는데
운영계 DB에는 별도 INDEXING 처리를 하지 않아 FULL TABLE SCAN을 타고 있었습니다.

따라서, 운영계 DB에 해당 컬럼에 대해 INDEXING 처리를 한 후 재테스트한 결과 어느정도 속도는 줄였지만, 여전히 사용자가 느끼기에는 느린 속도였습니다.

이를 해결하기 위해 Oracle에서 사용할 수 있는 Hint를 적용했습니다.
해당 테이블에는 PK 뿐 아니라 여러 

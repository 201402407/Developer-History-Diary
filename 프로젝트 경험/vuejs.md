...
우선 각 카드 신청 개발자들과 파트 담당자에게 약관 개편에 대해 설명을 듣고, 이를 바탕으로 약관 리스트 / 상세 / 아코디언 그룹의 컴포넌트 구조를 설계했습니다.

다음으로, 약관 정보 List를 하나의 interface로 설계하고 상/하위 Depth 약관 체크, 약관내용 데이터 맵핑 등 약관 관련 기능들을 하나의 클래스로 담고, Mixins를 활용해 다중상속을 받아 사용했습니다. 또, Props를 통한 카드신청 별 약관 분기처리와 vuex store를 활용해 약관동의여부 데이터를 관리했습니다. 그 후, Button, Input, Radio, checkbox 등 사용자 활용 UI 컴포넌트에 카드 신청 별 고유 ID 값을 채번하는 함수를 만들어 라우터 별 GA 태깅과 Tealeaf 분석을 위한 데이터를 추출해서 업무계 파트에 전달했습니다. 마지막으로, Enum과 주석을 사용해 가독성을 높였고, 동료 개발자들이 더욱 이해하기 쉽게 샘플 가이드도 제작했습니다.
...
이를 해결하기 위해 axios 설정 변경 대신 head tag에 script tag를 동적으로 생성하고, src를 연결해서 렌더링 시 호출되게 구현했습니다. 이후, windows 객체에 저장되어있는 기기 목록 변수값을 코드에서 호출(하기 위해 전역 인터페이스에 변수 이름을 추가하여 컴파일 타임에 Eslint를 통과시켰고)했고, 테스트 결과 정상 동작했습니다.

그당시엔 잘 몰랐는데, 지금 생각해보면 JSONP(JSON Padding)를 사용해서 script tag 변수에 url을 적용한 후 DOM에 추가시켜 호출했기 때문에 CORS 이슈없이 송수신이 가능하지 않았나 싶습니다.
(JSONP는 일종의 CORS 우회 방법으로, 서버에 JSONP가 적용되어있어야함. 만약, JSONP가 없다면 대부분의 jquery로 import해오는 것들은 오류가 날 것이다.)
